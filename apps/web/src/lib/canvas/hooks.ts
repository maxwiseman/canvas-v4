...
export function useCourses(options: CanvasAutoSyncOptions = {}) {
	...
	return {
		...
		isStale: false,
		refresh,
		setIcon: canvas.courses.setIcon,
		setNickname: canvas.courses.setNickname,
	};
}
...
export function useCourse(
	...
	const stamp = courseId
		? canvas.snapshot.syncManifest[0]?.hydratedScopes.courseAssignments[
				courseId
			]
		: undefined;
	const syncScope = `course:${courseId}`;
	const error = useMemo(
		() => getLatestSyncError(canvas, syncScope),
		[canvas, syncScope],
	);
	const hasAttemptedLoad = Boolean(stamp || error);
	const isMissing = Boolean(courseId && !course);
	const isStale = Boolean(
		courseId && revalidateIfStale && isScopeStale(stamp, staleMs),
	);
	const shouldRefresh =
		enabled &&
		Boolean(courseId) &&
		((isMissing && !hasAttemptedLoad) || isStale);
	const isRefreshing = useMemo(
		() =>
			canvas.snapshot.syncJobs.some(
				(job) => job.status === "running" && job.scope === syncScope,
			),
...
export function useAssignmentDetail(
	...
	const assignment =
		courseId && assignmentId
			? canvas.assignments.get(courseId, assignmentId)
			: undefined;
	const submission =
		courseId && assignmentId
			? canvas.submissions
					.listByCourse(courseId)
					.find((item) => String(item.assignment_id) === String(assignmentId))
			: undefined;
	const stamp =
		courseId && assignmentId
			? canvas.snapshot.syncManifest[0]?.hydratedScopes.assignmentDetails[
					`${courseId}:${assignmentId}`
				]
			: undefined;
	...
	useAutoRefresh({
		enabled:
			enabled && Boolean(courseId && assignmentId) && (isMissing || isStale),
		refresh,
...
export function useCourseModules(
	...
	const stamp = courseId
		? canvas.snapshot.syncManifest[0]?.hydratedScopes.modules[courseId]
		: undefined;
	...
	const isRefreshing = useMemo(
		() =>
			canvas.snapshot.syncJobs.some(
				(job) =>
					job.status === "running" && job.scope === `modules:${courseId}`,
			),
...
export function useCourseAnnouncements(
	...
	const announcements = courseId
		? canvas.announcements.listByCourse(courseId)
		: [];
	const stamp = courseId
		? canvas.snapshot.syncManifest[0]?.hydratedScopes.announcements?.[courseId]
		: undefined;
	...
	const isRefreshing = useMemo(
		() =>
			canvas.snapshot.syncJobs.some(
				(job) =>
					job.status === "running" && job.scope === `announcements:${courseId}`,
			),
		[canvas.snapshot.syncJobs, courseId],
	);
	const refresh = useCallback(async () => {
...
export function useModuleItemsByModule(
	...
	const moduleItems =
		courseId && moduleId
			? canvas.moduleItems.listByModule(courseId, moduleId)
			: [];
...
export function useAssignmentGroupsByCourse(
	courseId: string | undefined,
	options: CanvasAutoSyncOptions = {},
) {
	const courseAssignments = useCourseAssignments(courseId, options);
	const canvas = useCanvasData();
	const assignmentGroups = courseId
		? canvas.assignmentGroups.listByCourse(courseId)
		: [];
...
export function useSubmissionsByCourse(
	courseId: string | undefined,
	options: CanvasAutoSyncOptions = {},
) {
	const courseAssignments = useCourseAssignments(courseId, options);
...
export function useCanvasMutationQueue(operation?: string) {
	...
	const mutations = useMemo(
		() =>
			canvas.pendingMutations.filter(
				(mutation) => !operation || mutation.operation === operation,
			),
	...
	const submit = useCallback(
		(params: CreateSubmissionParams) => {
			if (!courseId || !assignmentId)
				throw new Error(
					"courseId and assignmentId are required to submit an assignment.",
				);
...
export function useCreateDiscussionEntry(
	courseId: string | undefined,
	...
	const createEntry = useCallback(
		(params: CreateDiscussionEntryParams) => {
			if (!courseId || !topicId)
				throw new Error(
					"courseId and topicId are required to create a discussion entry.",
				);
			return mutationQueue.queue({
				namespace: "canvas",
	...
	const createReply = useCallback(
		(params: CreateDiscussionEntryParams) => {
			if (!courseId || !topicId || !entryId) {
				throw new Error(
					"courseId, topicId, and entryId are required to create a discussion reply.",
				);
			}
			return mutationQueue.queue({
				namespace: "canvas",
...
function isSyncRunning(
	canvas: ReturnType<typeof useCanvasData>,
	scope: string,
): boolean {
	return canvas.snapshot.syncJobs.some(
		(job) => job.status === "running" && job.scope === scope,
	);
}

function getLatestSyncError(
	canvas: ReturnType<typeof useCanvasData>,
	scope: string,
) {
