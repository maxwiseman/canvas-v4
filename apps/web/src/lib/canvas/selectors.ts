...
export type CanvasCourseWithApp = CanvasCourse & {
	app: {
		icon: string | null;
	};
};
...
export function listCourses(
	...
	return snapshot.courses
		.map((course) => withCourseApp(snapshot, course))
		.sort((a, b) =>
			String(a.name ?? a.course_code ?? a.sync.canvasId).localeCompare(
				String(b.name ?? b.course_code ?? b.sync.canvasId),
			),
		);
}
...
export function listAssignmentsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasAssignment[] {
...
export function getAssignment(
	...
	return listAssignmentsByCourse(snapshot, courseId).find(
		(assignment) =>
			assignment.sync.canvasId === String(assignmentId) ||
			String(assignment.id) === String(assignmentId),
	);
}

export function listAssignmentGroupsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasAssignmentGroup[] {
...
export function listSubmissionsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasSubmission[] {
	return snapshot.submissions
		...
		.sort((a, b) =>
			String(b.submitted_at ?? b.graded_at ?? "").localeCompare(
				String(a.submitted_at ?? a.graded_at ?? ""),
			),
		);
}
...
export function listEnrollmentsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasEnrollment[] {
	return snapshot.enrollments.filter(
		(enrollment) => enrollment.sync.courseId === courseId,
	);
}

export function listModulesByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasModule[] {
...
export function listModuleItemsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasModuleItem[] {
...
export function listModuleItemsByModule(
	...
	return listModuleItemsByCourse(snapshot, courseId).filter(
		(item) => String(item.module_id) === String(moduleId),
	);
}

export function listAnnouncementsByCourse(
	snapshot: CanvasDebugSnapshot,
	courseId: string,
): CanvasAnnouncement[] {
...
export function listTodos(snapshot: CanvasDebugSnapshot): CanvasTodo[] {
	return [...snapshot.canvasTodos].sort((a, b) =>
		String(a.assignment?.due_at ?? "").localeCompare(
			String(b.assignment?.due_at ?? ""),
		),
	);
}
...
export function listCalendarEvents(
	...
	return [...snapshot.calendarEvents].sort((a, b) =>
		String(a.start_at ?? "").localeCompare(String(b.start_at ?? "")),
	);
}

function withCourseApp(
	snapshot: CanvasDebugSnapshot,
	course: CanvasCourse,
): CanvasCourseWithApp {
	const overlay = snapshot.courseOverlays.find(
		(item) => item.canvasCourseId === course.sync.canvasId,
	);
	return {
		...course,
		app: {
