...
export async function loadCanvasSnapshot(ownerId: string): Promise<CanvasDebugSnapshot> {
  const row = await getDb().snapshots.get(canvasStorageId(ownerId));
  return row?.snapshot ?? emptyDebugSnapshot();
}

export async function persistCanvasSnapshot(ownerId: string, snapshot: CanvasDebugSnapshot): Promise<void> {
...
export async function clearCanvasSnapshot(ownerId: string): Promise<CanvasDebugSnapshot> {
  await getDb().snapshots.delete(canvasStorageId(ownerId));
  return emptyDebugSnapshot();
}

export async function loadMutationQueue(ownerId: string): Promise<LocalMutation[]> {
  return getDb().mutations.where("ownerId").equals(ownerId).sortBy("updatedAt");
}

export async function persistMutationQueue(ownerId: string, mutations: LocalMutation[]): Promise<void> {
  await getDb().transaction("rw", getDb().mutations, async () => {
    await getDb().mutations.where("ownerId").equals(ownerId).delete();
    if (mutations.length > 0) await getDb().mutations.bulkPut(mutations.map((mutation) => ({ ...mutation, ownerId })));
  });
}
