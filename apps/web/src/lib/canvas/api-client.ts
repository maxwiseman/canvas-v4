...
export async function syncCourseBasics(data: { courseId: string }): Promise<CanvasEnvelope> {
...
export async function syncCourseModules(data: { courseId: string }): Promise<CanvasEnvelope> {
...
async function fetchCanvasEnvelope(input: string, init?: RequestInit): Promise<CanvasEnvelope> {
  ...
  if (!response.ok) {
    throw new Error("error" in data && data.error ? data.error : `Canvas sync request failed: ${response.status}`);
  }
