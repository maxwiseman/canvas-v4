...
import { Input } from "@canvas-v4/ui/components/input";
import { Separator } from "@canvas-v4/ui/components/separator";
import {
	Sheet,
	SheetContent,
	SheetDescription,
	SheetHeader,
	SheetTitle,
} from "@canvas-v4/ui/components/sheet";
import { Skeleton } from "@canvas-v4/ui/components/skeleton";
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
} from "@canvas-v4/ui/components/tooltip";
import { useIsMobile } from "@canvas-v4/ui/hooks/use-mobile";
import { cn } from "@canvas-v4/ui/lib/utils";
...
function SidebarGroupLabel({
	...
	return useRender({
		...
		props: mergeProps<"div">(
			{
				className: cn(
					"flex h-8 shrink-0 items-center rounded-xl px-2 text-muted-foreground text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 [&>svg]:size-4 [&>svg]:shrink-0",
					className,
				),
			},
			props,
		...
		props: mergeProps<"button">(
			{
				className: cn(
					...
					"h-8 rounded-sm p-2 [&_svg]:text-muted-foreground",
					className,
				),
			},
			props,
