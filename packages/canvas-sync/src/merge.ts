...
export function upsertBySyncId<T extends CanvasEntity<object>>(current: T[], incoming: T[]): T[] {
...
function mergeEntity<T extends CanvasEntity<object>>(existing: T, incoming: T): T {
  return {
    ...existing,
    ...incoming,
    sync: {
...
export function mergeEnvelope(snapshot: CanvasDebugSnapshot, envelope: CanvasEnvelope): CanvasDebugSnapshot {
  const manifest = updateManifest(snapshot.syncManifest[0] ?? createEmptyManifest(), envelope);
  return {
    ...snapshot,
    users: upsertBySyncId(snapshot.users, envelope.users ?? []),
    courses: upsertBySyncId(snapshot.courses, envelope.courses ?? []),
    enrollments: upsertBySyncId(snapshot.enrollments, envelope.enrollments ?? []),
    assignments: upsertBySyncId(snapshot.assignments, envelope.assignments ?? []),
    submissions: upsertBySyncId(snapshot.submissions, envelope.submissions ?? []),
    assignmentGroups: upsertBySyncId(snapshot.assignmentGroups, envelope.assignmentGroups ?? []),
    modules: upsertBySyncId(snapshot.modules, envelope.modules ?? []),
    moduleItems: upsertBySyncId(snapshot.moduleItems, envelope.moduleItems ?? []),
    pages: upsertBySyncId(snapshot.pages, envelope.pages ?? []),
    announcements: upsertBySyncId(snapshot.announcements, envelope.announcements ?? []),
    calendarEvents: upsertBySyncId(snapshot.calendarEvents, envelope.calendarEvents ?? []),
    canvasTodos: upsertBySyncId(snapshot.canvasTodos, envelope.canvasTodos ?? []),
    syncErrors: upsertErrors(snapshot.syncErrors, envelope.syncMeta.errors ?? []),
    syncManifest: [manifest],
  };
}

function upsertErrors(current: CanvasSyncError[], incoming: CanvasSyncError[]): CanvasSyncError[] {
  if (incoming.length === 0) return current;
...
function updateManifest(manifest: SyncManifest, envelope: CanvasEnvelope): SyncManifest {
  const stamp = {
  ...
  const next: SyncManifest = {
    ...manifest,
    activeCourseIds: envelope.courses?.map((course) => course.sync.canvasId) ?? manifest.activeCourseIds,
    hydratedScopes: {
      dashboard: envelope.syncMeta.scope === "dashboard" || envelope.syncMeta.scope === "bootstrap" ? stamp : manifest.hydratedScopes.dashboard,
...
export function isScopeStale(stamp: { fetchedAt?: string } | undefined, maxAgeMs: number, now = Date.now()): boolean {
...
export function collectionCount(snapshot: CanvasDebugSnapshot, name: CanvasCollectionName): number {
